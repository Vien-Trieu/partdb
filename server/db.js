...
function loadDotEnvFromPossibleLocations() {
  ...
  console.warn(
    "⚠️ No env.production found in snapshot or filesystem; falling back to process.env values.",
  );
  return null;
}
...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
  console.warn(
    "⚠️ SUPABASE_URL or SUPABASE_ANON_KEY are missing from env. They must be present for Supabase to work.",
  );
}
...
console.log(
  "⚠️ NODE_TLS_REJECT_UNAUTHORIZED =",
  process.env.NODE_TLS_REJECT_UNAUTHORIZED,
);
